...
If you're about to usa a wildcard when matching on a large but finite set of possibilities because you have many possibilities that return the same thing, consider reaching for disjunctive patterns instead. That is you can replace:
...
```
This still requires a bit more work than a wildcard, but we promise it'll be worth when you refactor your code.
